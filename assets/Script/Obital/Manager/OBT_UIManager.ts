...
export default class OBT_UIManager {
    ...
    //     } catch (error) {
    //         console.info(`[OBT_UIManager]:showUI:prefab ${uiName} doesn't have script ${scriptName}`);
    //     }
    // }
    ...
    public loadUINode(uiUrl: string, scriptName?: string): Node {
        ...
        let uiPrefab = OBT.instance.resourceManager.getAssets(abName, `Prefabs/${uiUrl}`) as Prefab;
        if (!uiPrefab) {
            return null;
        }
        const uiNode: Node = instantiate(uiPrefab);
