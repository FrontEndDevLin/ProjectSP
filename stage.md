...
每个道具有一个类
  ...
  特殊道具(有特殊效果)有[独立道具类]
独立道具类继承[通用独立道具基类]
每挂载一个道具, 将该道具的类注册到ItemManager里
在定制事件触发时, 通过[即时事件中心]传递给ItemManager
ItemManager将事件分发给所有特殊道具类处理

将[核心升级包]视为一类道具, 改在Items.json中定义

将[核心]中的buff_list和特性集成为道具

废弃[traits]相关代码

核心选择界面时, 核心中的道具没有实例化, 无法获得介绍文本
